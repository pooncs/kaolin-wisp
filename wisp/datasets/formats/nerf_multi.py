...
def load_nerf_standard_data(root, split='train', bg_color='white', num_workers=-1, mip=None):
    ...
    if 'x_fov' in transform_dict[split]:
        # Degrees
        x_fov = transform_dict[split]['frames'][0]
        ...
        if 'y_fov' in transform_dict[split]:
            ...
            fy = fx
    elif 'fl_x' in transform_dict[split]['frames'][0]:
        fx = [float(x['fl_x']) / float(2**mip) for x in transform_dict[split]['frames']]
        if 'fl_y' in transform_dict[split]['frames'][0]:
            fy = [float(x['fl_y']) / float(2**mip) for x in transform_dict[split]['frames']]
        else:
            fy = fx
    elif 'camera_angle_x' in transform_dict[split]['frames'][0]:
        # Radians
        camera_angle_x = [float(x['camera_angle_x']) for x in transform_dict[split]['frames']]
        
        fx = (0.5 * w) / np.tan(0.5 * camera_angle_x)

        if 'camera_angle_y' in transform_dict[split]:
            camera_angle_y = [float(x['camera_angle_y']) for x in transform_dict[split]['frames']]
    ...
    if 'cx' in transform_dict[split]['frames'][0]:
        x0 = [(float(x['cx']) / (2**mip)) - (w//2) for x in transform_dict[split]['frames']]
    if 'cy' in transform_dict[split]['frames'][0]:
        y0 = [(float(x['cy']) / (2**mip)) - (h//2) for x in transform_dict[split]['frames']]
